defmodule Util.MapTest do
  ...
  describe "has_only" do
    test "true with string keys" do
      assert Map.has_only(%{"a" => 4}, MapSet.new(["a"]))
    end
    ...
    test "true with arbitrary order" do
      assert Map.has_only(%{a: 4, b: 5}, MapSet.new([:b, :a]))
    end

    test "false on too many keys" do
      assert !Map.has_only(%{"a" => 4, "b" => 5}, MapSet.new(["a"]))
    end

    test "false on too few keys" do
      assert !Map.has_only(%{}, MapSet.new(["a"]))
    end

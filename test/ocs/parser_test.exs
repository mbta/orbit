defmodule OCS.ParserTest do
  use ExUnit.Case, async: true

  alias OCS.Utilities.Time, as: OCSTime
  ...
  describe "parse/2" do
    test "parses schedule (TSCH) message" do
      ...
      assert OCS.Parser.parse(line, OCSTime.in_ocs_tz(~N[2017-03-17 02:00:08])) ==
               {:ok,
    ...
    test "ignores train movement (TMOV) message" do
      ...
      assert OCS.Parser.parse(line, time_today) == {:ok, :ignored}
    end
    ...
    test "handles empty message" do
      assert OCS.Parser.parse("", Timex.local()) == {:error, %MatchError{term: [""]}}
    end

    test "handles non-integer message counter" do
      assert OCS.Parser.parse("not_an_integer,DEVI,02:04:55,B,SW,WON_SW1,0,1,0,1", Timex.local()) ==
               {:error, %MatchError{term: :error}}
    end

    test "handles message type not defined" do
      assert OCS.Parser.parse("4364,NOTD,02:04:55,B,SW,WON_SW1,0,1,0,1", Timex.local()) ==
               {:error,
                %RuntimeError{
    ...
    test "handles message in unexpected format" do
      assert OCS.Parser.parse("4364,,02:04:55,", Timex.local()) ==
               {:error,
                %RuntimeError{
  ...
  describe "convert_ocs_car_number/2" do
    test "converts Red Line 25xx car number to 15xx" do
      assert OCS.Parser.convert_ocs_car_number("R", "2501") == "1501"
    end

    test "doesn't convert other line 25xx car number to 15xx" do
      assert OCS.Parser.convert_ocs_car_number("O", "2501") == "2501"
    end
  end
end

defmodule OCS.Message.TschMessageTest do
  ...
  describe "parse/2" do
    test "handles bad arguments" do
      assert {:error, _error} =
               OCS.Parser.TschMessage.parse(:foo, @test_time)
    end

    test "parses TSCH CON message" do
      assert OCS.Parser.TschMessage.parse(
               {4331, :tsch, @test_time,
    ...
    test "parses TSCH RLD message (old style)" do
      assert OCS.Parser.TschMessage.parse({3081, :tsch, @test_time, ["O", "RLD"]}, @test_time) ==
               {:ok,
                %OCS.Message.TschRldMessage{
                  counter: 3081,
                  timestamp: @test_time,

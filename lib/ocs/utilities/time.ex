defmodule OCS.Utilities.Time do
  ...
  def local_now(timezone \\ Application.get_env(:orbit, :timezone)) do
  ...
  def to_gtfs_time(time, current_time) do
    with {:ok, current_dt} <-
         ...
         {:ok, dt} <- DateTime.shift_zone(dt, Application.get_env(:orbit, :timezone)) do
  ...
  def maybe_to_unix(dt),
    do: dt |> naive_to_datetime(Application.get_env(:orbit, :timezone)) |> DateTime.to_unix()
end

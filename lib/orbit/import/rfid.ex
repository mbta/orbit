defmodule Orbit.Import.Rfid do
  ...
  def import_rows(rows) do
    ...
    {:ok, imported_count} =
      Repo.transaction(fn -> import_employees(serials_by_badge) end)

    Logger.info("rfid_import count=#{imported_count}")
  end

  defp import_employees(serials_by_badge) do
    for {badge_number, badge_serials} <- serials_by_badge, reduce: 0 do
      acc ->
        case Repo.get_by(Employee, badge_number: badge_number) do
          ...
          employee ->
            acc + import_employee(employee, badge_serials)
        end
    end
  end

  defp import_employee(employee, badge_serials) do
